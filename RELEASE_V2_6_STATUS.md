...
10. 설정된 `maxStackLevels`는 후보 상한으로 사용하되 실제 컨테이너 높이·상부하중·면적지지를 모두 통과해야 다단 적층
11. 각대/랩핑 등 활성 포장재의 예약 중량과 높이를 최대중량/천장 검사에 포함
12. 실제 컨테이너 슬롯이 없는 팔레트는 원점 중복 배치하지 않고 미적재 반환
