...
세부 소유권:
- 제품 검색 결과 스크롤/고정 헤더/모바일 6단계 rail: `guided-workflow-v2.css`
- 적재 전략 카드/전략 반응형/모션 감소: `guided-loading-strategy.css`
- 박스·파렛트 적재 유형 카드/자동 적재 상태 배지: `guided-loading-unit.css`
- 결과 탭 및 최종 결과 보조 UI: `guided-workflow.css`와 `guided-result-tabs-enhancer.css`

### 박스 / 파렛트 기본 모드
- `src/mode.css`
- `.mode-tabs`와 기본 박스/파렛트 모드 선택 표현을 소유합니다.
- 가이드 review CSS에서 `.mode-tabs`를 다시 꾸미지 않습니다.
...
- 이미 기능 CSS로 이전한 selector를 review CSS에 다시 넣지 않음
- 삭제된 `transport-equipment-scroll-fix.css`와 import의 재도입 금지
