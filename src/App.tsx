...
const emptyDraft: CargoDraft = { id: '', name: '', length: 0.5, width: 0.4, height: 0.3, weightKg: 10, quantity: 1, maxStackLayers: 7, maxTopLoadKg: 100 };
...
function Scene({ result, container }: { result: LoadingResult; container: ContainerSpec }) {
  ...
  return <>
    ...
    {zoneBoundaries.map((x, index) => {
      const sx = x * scale - (container.length * scale) / 2;
      return <mesh key={x} position={[sx, cy, 0]}><boxGeometry args={[0.012, container.height * scale, container.width * scale]} /><meshBasicMaterial transparent opacity={0.18} /></mesh>;
    })}
    ...
    {result.placements.map((p, index) => {
      ...
      const pos: [number, number, number] = [(p.x + p.length / 2) * scale - container.length * scale / 2, (p.z + p.height / 2) * scale, (p.y + p.width / 2) * scale - container.width * scale / 2];
      return <group key={`${p.cargoId}-${index}`}>
        <mesh position={pos}><boxGeometry args={[p.length * scale, p.height * scale, p.width * scale]} /><meshStandardMaterial color={cargoColor(p.cargoId)} roughness={0.65} metalness={0.04} /></mesh>
        {index < 80 && <Text position={[pos[0], pos[1] + p.height * scale / 2 + 0.035, pos[2]]} fontSize={0.055} anchorX="center" anchorY="bottom">{`R${a.row} C${a.column} L${a.layer}`}</Text>}
      </group>;
    })}
...
export default function App() {
  ...
  const saveCargo = () => {
    ...
    const next: CargoItem = { ...draft, id, name, quantity: Math.floor(draft.quantity), maxStackLayers: draft.maxStackLayers ? Math.max(1, Math.floor(draft.maxStackLayers)) : undefined, maxTopLoadKg: draft.maxTopLoadKg || undefined };
  ...
  const editCargo = (item: CargoItem) => { setEditingId(item.id); setDraft({ ...item, maxStackLayers: item.maxStackLayers ?? 7, maxTopLoadKg: item.maxTopLoadKg ?? 0 }); };
  ...
  const loadLocal = () => { const state = readStoredState(); if (!state) return setSaveMessage('저장된 데이터가 없습니다.'); setContainer(state.container); setCargo(state.cargo); setResult(loadContainer(state.container, state.cargo.filter(item => item.quantity > 0))); setSaveMessage('저장된 데이터를 불러왔습니다.'); };
  ...
  return <main className="app-shell">
    <header className="topbar"><div><strong>Container Loading Simulator</strong><span>Codex development v0.11.0</span></div><div className="top-actions"><button className="secondary" onClick={saveLocal}>저장</button><button className="secondary" onClick={loadLocal}>불러오기</button><button onClick={runLoading}>적재 실행</button></div></header>
    <section className="workspace">
      <aside className="panel left-panel">
        <h2>컨테이너 설정</h2><div className="form-grid container-form">
          <label>길이(m)<input type="number" step="0.01" value={container.length} onChange={e => updateContainer('length', e.target.value)} /></label><label>폭(m)<input type="number" step="0.01" value={container.width} onChange={e => updateContainer('width', e.target.value)} /></label><label>높이(m)<input type="number" step="0.01" value={container.height} onChange={e => updateContainer('height', e.target.value)} /></label><label>최대중량(kg)<input type="number" step="100" value={container.maxPayloadKg} onChange={e => updateContainer('maxPayloadKg', e.target.value)} /></label>
        </div><div className="form-actions utility-actions"><button className="secondary" onClick={saveLocal}>현재 데이터 저장</button><button className="danger" onClick={resetAll}>전체 초기화</button></div>{saveMessage && <p className="muted status-message">{saveMessage}</p>}
        <h2>{editingId ? '박스 수정' : '박스 등록'}</h2><div className="cargo-form"><label>코드<input value={draft.id} onChange={e => updateDraft('id', e.target.value)} disabled={Boolean(editingId)} /></label><label>이름<input value={draft.name} onChange={e => updateDraft('name', e.target.value)} /></label><div className="form-grid">
          <label>길이(m)<input type="number" step="0.01" value={draft.length} onChange={e => updateDraft('length', e.target.value)} /></label><label>폭(m)<input type="number" step="0.01" value={draft.width} onChange={e => updateDraft('width', e.target.value)} /></label><label>높이(m)<input type="number" step="0.01" value={draft.height} onChange={e => updateDraft('height', e.target.value)} /></label><label>중량(kg)<input type="number" value={draft.weightKg} onChange={e => updateDraft('weightKg', e.target.value)} /></label><label>수량<input type="number" value={draft.quantity} onChange={e => updateDraft('quantity', e.target.value)} /></label><label>최대 적층단<input type="number" value={draft.maxStackLayers ?? 1} onChange={e => updateDraft('maxStackLayers', e.target.value)} /></label><label>상부허용(kg)<input type="number" value={draft.maxTopLoadKg ?? 0} onChange={e => updateDraft('maxTopLoadKg', e.target.value)} /></label>
        </div><div className="form-actions"><button onClick={saveCargo}>{editingId ? '수정 저장' : '박스 추가'}</button>{editingId && <button className="secondary" onClick={resetDraft}>취소</button>}</div></div>
        <h2>대기 화물 <span className="section-count">{waitingCount} EA</span></h2>{cargo.length === 0 ? <p className="muted">등록된 박스가 없습니다.</p> : cargo.map(item => <article className="cargo-card" key={item.id}><div className="cargo-head"><b>{item.name}</b><span>{item.id}</span></div><span>{item.length}×{item.width}×{item.height} m · {item.weightKg} kg/EA</span><span>최대 {item.maxStackLayers ?? '-'}단 · 상부 {item.maxTopLoadKg ?? '-'} kg</span><div className="quantity-row"><button className="mini" onClick={() => changeQuantity(item.id,-1)}>-</button><strong>{item.quantity} EA</strong><button className="mini" onClick={() => changeQuantity(item.id,1)}>+</button></div><div className="card-actions"><button className="secondary" onClick={() => editCargo(item)}>수정</button><button className="danger" onClick={() => deleteCargo(item.id)}>삭제</button></div></article>)}
      </aside>
      ...
      <aside className="panel right-panel">
        <h2>적재 결과</h2><div className="metric"><span>적재 수량</span><strong>{result.placements.length}</strong></div><div className="metric"><span>적재 중량</span><strong>{result.loadedWeightKg.toLocaleString()} kg</strong></div><div className="metric"><span>사용 CBM</span><strong>{result.usedVolumeM3.toFixed(2)} m³</strong></div><div className="metric"><span>체적 적재율</span><strong>{fillRate.toFixed(1)}%</strong></div>
        <h2>위치 체계</h2><div className="location-key"><b>R = 행 · C = 열 · L = 단</b><span>행은 안쪽→문쪽, 열은 좌→우, 단은 바닥→위 순서입니다.</span></div><div className="location-list">{locationRows.map(({p,a},i) => <div className="location-row" key={`${p.cargoId}-${i}`}><b>{`R${a.row} C${a.column} L${a.layer}`}</b><span>{p.cargoId}</span><small>{a.zone}</small></div>)}</div>{result.placements.length > 40 && <p className="muted">앞 40개 위치만 표시합니다. 3D 화면에는 앞 80개 좌표 라벨을 표시합니다.</p>}
        <h2>무게중심·품질 평가</h2><div className="quality-score"><span>종합 품질</span><strong>{quality.loadingQualityScore.toFixed(0)}점 · {quality.grade}등급</strong></div><div className="metric"><span>균형 점수</span><strong>{quality.balanceScore.toFixed(0)}</strong></div><div className="metric"><span>안정성 점수</span><strong>{quality.stabilityScore.toFixed(0)}</strong></div><div className="metric"><span>앞뒤 중심 편차</span><strong>{quality.longitudinalDeviationPct.toFixed(1)}%</strong></div><div className="metric"><span>좌우 중심 편차</span><strong>{quality.lateralDeviationPct.toFixed(1)}%</strong></div><div className="metric"><span>무게중심 높이</span><strong>{quality.verticalCenterPct.toFixed(1)}%</strong></div><article className="quality-note"><b>무게중심 좌표</b><span>X {quality.centerOfGravity.x.toFixed(2)}m · Y {quality.centerOfGravity.y.toFixed(2)}m · Z {quality.centerOfGravity.z.toFixed(2)}m</span></article>{quality.messages.map((m,i) => <p className="quality-message" key={i}>• {m}</p>)}
        <h2>자동 검증</h2>{result.validationIssues.length === 0 ? <p className="muted">충돌 및 경계 침범 없음</p> : result.validationIssues.map((issue,index) => <article className="warning-card" key={`${issue.type}-${index}`}><b>{issue.type}</b><span>{issue.message}</span></article>)}
        <h2>미적재 화물</h2>{result.remaining.length === 0 ? <p className="muted">없음</p> : result.remaining.map(item => <article className="warning-card" key={item.cargoId}><b>{item.cargoId} · {item.quantity} EA</b><span>{item.reason}</span></article>)}
      </aside>
    </section>
  </main>;
}
