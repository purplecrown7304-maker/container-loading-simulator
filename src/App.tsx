...
export default function App() {
  ...
  const saveCargo = () => {
    ...
    setSaveMessage(editingId ? `${id} 수정 완료 · 자동 적재를 다시 실행하세요.` : `${id} 등록 완료 · 자동 적재를 실행하세요.`);
    resetDraft();
  };
  ...
  const runLoading = () => {
    ...
    if (mode === 'pallets') { setPalletRunToken(token => token + 1); return; }
    setIsRunning(true);
    setSaveMessage('적재 계산 중…');
    window.setTimeout(() => {
      const nextResult = loadContainer(container, cargo.filter(item => item.quantity > 0));
      setResult(nextResult);
      setIsRunning(false);
      setSaveMessage(`계산 완료 · ${nextResult.placements.length}EA 적재`);
    }, 20);
  };
  ...
  return <main className="app-shell mockup-dashboard">
    <header className="topbar mockup-topbar">
      ...
      <nav className="main-nav" aria-label="주요 메뉴">
        <button className="nav-item active">▣ 대시보드</button><button className="nav-item">⬡ 3D 뷰</button><button className="nav-item">▥ 2D 레이아웃</button><button className="nav-item" onClick={showResults}>▤ 결과 보기</button>
      </nav>
    ...
    <section className="dashboard-grid">
      ...
      <section className="dashboard-center">
        <section className="dashboard-card viewer-card"><div className="card-heading-row"><div><h2>적재 시뮬레이션 미리보기</h2><div className="cargo-legend">{cargo.slice(0,8).map(item=><span key={item.id}><i style={{background:cargoColor(item.id)}}/>{item.id}</span>)}<span><i className="empty"/>여유 공간</span></div></div><div className="viewer-toolbar"><button className="active">3D 뷰</button><button>2D 뷰</button></div></div><div className="viewer-host">{isRunning&&<div className="calculation-overlay"><b>적재 계산 중</b><span>화물 배치와 안전조건을 계산하고 있습니다.</span></div>}<Suspense fallback={<LoadingFallback />}>{mode==='boxes'?<BoxLoadingViewer result={result} container={container}/>:<section className="viewer pallet-viewer"><PalletModePanel container={container} cargo={cargo} runToken={palletRunToken}/></section>}</Suspense></div><div className="viewer-bottom-actions"><button className="result-open-action" onClick={showResults}>▤ 결과 보기</button><span>층별 보기 · 하중 분포 · 적재 리스트 · 고급 분석은 결과창에서 확인</span></div></section>
      </section>

      <aside className="dashboard-right">
        <section className="dashboard-card summary-card"><h2>4. 적재 요약</h2><div className="summary-metric-grid"><div><span>총 부피</span><b>{result.usedVolumeM3.toFixed(1)} / {totalVolume.toFixed(1)} m³</b><small>{fillRate.toFixed(1)}%</small></div><div><span>총 중량</span><b>{result.loadedWeightKg.toLocaleString()} / {container.maxPayloadKg.toLocaleString()} kg</b><small>{weightRate.toFixed(1)}%</small></div><div><span>사용 박스 수</span><b>{result.placements.length} EA</b></div><div><span>총 층수</span><b>{maxLayer} 층</b></div></div><button className="constraint-ok" onClick={showResults}>✓ 결과 상세 보기</button></section>
