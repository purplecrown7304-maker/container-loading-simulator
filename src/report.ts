...
export function buildLoadingReportHtml(container: ContainerSpec, cargo: CargoItem[], result: LoadingResult): string {
  const certification = workOrderCertificationForTarget({ mode: 'boxes', container, cargo, result });
  ...
  const workOrderSafeToExecute = approval === 'pass' || approval === 'caution';
  ...
  const finalTransportWeightKg = result.loadedWeightKg + securing.estimatedAddedWeightKg;
  ...
  return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>
    ...
    <div class="section-title"><h2>필요 보조자재</h2><span>${escapeHtml(securing.levelLabel)}</span></div><section class="materials">${materialCards}</section>
    <div class="section-title"><h2>관성 테스트 권장 사항</h2><span>${approval === 'danger' ? '위험 경고 · 수정 후 재검증 필수' : approval === 'incomplete' ? '검사 미완료 · 남은 검사 필요' : approval === 'caution' ? '주의 · 출고 전 보완 확인 필수' : '현장 최종 확인 사항'}</span></div><ol class="recommendations">${recommendationItems}</ol>
    ...
    <p class="technical-note">이 문서는 관성 PASS 여부와 관계없이 현재 적재 위치를 전달하기 위해 발행됩니다. 관성 ${escapeHtml(approvalLabel)}은 시뮬레이터 내부 비교 결과이며 실제 운송 안전 인증을 의미하지 않습니다. 위험 또는 검사 미완료 표시는 출고 승인과 다르므로 작업 전 실제 장비 제원, 포장 강도, 현장 결박 기준과 보조자재 규격을 확인하세요. 장비 기준: ${escapeHtml(equipment.sourceLabel)}.</p>
    <footer class="footer"><span>장비: ${escapeHtml(equipment.shortName)}</span><span>물리검증: ${physicsVerified ? '완료' : '미검증'}</span><span>관성: ${escapeHtml(approvalLabel)}</span><span>최종 운송중량: ${finalTransportWeightKg.toFixed(1)} kg</span></footer>
  </main></body></html>`;
}
...
export function openLoadingReport(container: ContainerSpec, cargo: CargoItem[], result: LoadingResult): boolean {
  if (!result.placements.length) {
    window.alert('작업지시서를 만들 적재 결과가 없습니다. 먼저 자동 적재를 실행하세요.');
    return true;
  }
