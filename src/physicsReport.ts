...
export function buildPhysicsReportHtml(
  ...
  const scenarioRows = physics.scenarios.map(row => `<tr><td>${scenarioLabel(row.scenario)}</td><td>${row.score}</td><td>${row.stableCount}</td><td>${row.warningCount}</td><td>${row.unstableCount}</td><td>${mm(row.maxHorizontalShiftM)}</td><td>${row.maxTiltDeg.toFixed(1)}°</td></tr>`).join('');
  ...
  *{box-sizing:border-box}body{font-family:Arial,'Noto Sans KR',sans-serif;margin:30px;color:#172033;font-size:12px}h1{font-size:24px;margin:0 0 6px}h2{font-size:16px;margin:24px 0 8px}p{color:#526477}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:16px 0}.card{border:1px solid #cbd8e6;border-radius:8px;padding:10px}.card span{display:block;color:#66788b;font-size:10px}.card b{display:block;font-size:16px;margin-top:4px}table{border-collapse:collapse;width:100%;font-size:10px}th,td{border:1px solid #cfd9e4;padding:6px;text-align:left}th{background:#edf3f9}.warn{color:#a16207}.danger{color:#b42318}.note{margin-top:20px;padding:10px;border:1px solid #d7e0ea;background:#f7fafc;font-size:10px;line-height:1.55}@media print{body{margin:12mm}.no-print{display:none}}</style></head><body>
  <h1>컨테이너 물리 안정성 검증 리포트</h1><p>Rapier 3D deterministic · 정적 중력 / 급제동 / 횡가속 종합검증</p>
  <div class="grid"><div class="card"><span>종합점수</span><b>${physics.score}/100</b></div><div class="card"><span>최악 조건</span><b>${scenarioLabel(physics.worstScenario)}</b></div><div class="card"><span>적재 박스</span><b>${loading.placements.length.toLocaleString()} EA</b></div><div class="card"><span>적재중량</span><b>${loading.loadedWeightKg.toLocaleString()} kg</b></div><div class="card"><span>컨테이너</span><b>${container.length.toFixed(2)}×${container.width.toFixed(2)}×${container.height.toFixed(2)}m</b></div><div class="card"><span>최대 수평 이동</span><b>${mm(physics.maxHorizontalShiftM)}</b></div><div class="card"><span>최대 기울기</span><b>${physics.maxTiltDeg.toFixed(1)}°</b></div><div class="card"><span>불안정 / 주의</span><b>${physics.unstableCount} / ${physics.warningCount}</b></div></div>
  <h2>시나리오별 결과</h2><table><thead><tr><th>조건</th><th>점수</th><th>안정</th><th>주의</th><th>불안정</th><th>최대 수평 이동</th><th>최대 기울기</th></tr></thead><tbody>${scenarioRows}</tbody></table>
  <h2>재확인 위치</h2>${issues.length ? `<table><thead><tr><th>#</th><th>코드</th><th>품목</th><th>판정</th><th>수평 이동</th><th>높이 변화</th><th>기울기</th><th>사유</th><th>XYZ(m)</th></tr></thead><tbody>${issueRows}</tbody></table>` : '<p>주의 또는 불안정 위치가 없습니다.</p>'}
