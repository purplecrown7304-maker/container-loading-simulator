...
function zoneHeightProfile(container: ContainerSpec, placements: Placement[], start: number, end: number) {
  ...
  return { averageHeightM: samples ? totalTop / samples : 0, maxHeightM };
}
...
export function assessZoneUtilization(container: ContainerSpec, placements: Placement[]): ZoneUtilization[] {
  ...
  return definitions.map(zone => {
    const usedVolumeM3 = placements.reduce((sum, p) => {
      ...
      return overlapX <= EPS ? sum : sum + overlapX * p.width * p.height;
    }, 0);
...
export function detectZoneFlowWarning(zones: ZoneUtilization[]): string | null {
  ...
  // 구체적인 높이 위험을 단순 적재율 흐름보다 먼저 알린다.
  if (middle && door) {
