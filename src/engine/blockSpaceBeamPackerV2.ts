...
function countOptions(max: number) {
  if (max < 1) return [];
  const middle = Math.max(1, Math.ceil(max * 0.75));
  return [...new Set([max, Math.max(1, max - 1), middle, Math.max(1, Math.ceil(max / 2)), 1])]
    .sort((a, b) => b - a);
}
...
function blocksFor(item: CargoItem, remaining: number, space: Space, allowSingles: boolean): Block[] {
  ...
  for (const orientation of orientations(item)) {
    ...
    // 같은 수량이면 컨테이너 폭 방향을 넓게 채우는 블록을 우선한다.
    variants.sort((a, b) =>
...
function occupied(candidate: Candidate): Placement {
  return {
    ...
    y: candidate.y,
    z: candidate.z,
    length: candidate.block.length,
    width: candidate.block.width,
...
function contactMetrics(candidate: Candidate, placements: Placement[]): ContactMetrics {
  const c = occupied(candidate);
  let allArea = 0;
  let sameSkuArea = 0;

  for (const p of placements) {
    let area = 0;
    const xOverlap = overlap1d(c.x, c.x + c.length, p.x, p.x + p.length);
    const yOverlap = overlap1d(c.y, c.y + c.width, p.y, p.y + p.width);
    const zOverlap = overlap1d(c.z, c.z + c.height, p.z, p.z + p.height);

    const xTouch = Math.abs(c.x - (p.x + p.length)) <= TOUCH || Math.abs(p.x - (c.x + c.length)) <= TOUCH;
    const yTouch = Math.abs(c.y - (p.y + p.width)) <= TOUCH || Math.abs(p.y - (c.y + c.width)) <= TOUCH;
    const zTouch = Math.abs(c.z - (p.z + p.height)) <= TOUCH || Math.abs(p.z - (c.z + c.height)) <= TOUCH;

    if (xTouch) area += yOverlap * zOverlap;
    if (yTouch) area += xOverlap * zOverlap;
    if (zTouch) area += xOverlap * yOverlap;
    if (area <= EPS) continue;

    allArea += area;
    if (p.cargoId === candidate.block.item.id) sameSkuArea += area;
  }
  return { allArea, sameSkuArea };
}

function remainingAfterCandidate(item: CargoItem, state: State, candidate?: Candidate) {
  const current = state.remaining.get(item.id) ?? 0;
  if (!candidate || candidate.block.item.id !== item.id) return current;
  return Math.max(0, current - candidate.block.quantity);
}
...
function spaceCanFitRemaining(space: Space, state: State, context: Context, candidate?: Candidate) {
  return context.cargo.some((item) => remainingAfterCandidate(item, state, candidate) > 0 && itemFitsSpace(item, space));
}

function bestTilingFill(space: Space, state: State, context: Context, candidate?: Candidate) {
  const area = Math.max(EPS, space.length * space.width);
  let best = 0;
  for (const item of context.cargo) {
    ...
    for (const o of orientations(item)) {
      ...
      const count = Math.min(left, nx * ny);
      const fill = count * o.boxLength * o.boxWidth / area;
      best = Math.max(best, Math.min(1, fill));
    }
  }
  return best;
}

function residualPenalty(candidate: Candidate, state: State, context: Context) {
  const block = candidate.block;
  const space = candidate.space;
  const residualWidth = Math.max(0, space.width - block.width);
  const residualLength = Math.max(0, space.length - block.length);
  let penalty = 0;

  if (residualWidth > EPS) {
    const side: Space = {
      x: space.x,
      y: space.y,
      z: space.z,
    ...
    const fill = bestTilingFill(side, state, context, candidate);
    const ratio = residualWidth / Math.max(EPS, context.container.width);
    if (!spaceCanFitRemaining(side, state, context, candidate)) penalty += 260 * ratio + 70;
    else penalty += (1 - fill) * 120 * ratio;

    const aspect = Math.max(side.length, side.width) / Math.max(EPS, Math.min(side.length, side.width));
    if (aspect > 5 && fill < 0.75) penalty += Math.min(140, (aspect - 5) * 12 * (1 - fill));
  }

  if (residualLength > EPS) {
    const front: Space = {
      x: space.x + block.length,
      y: space.y,
      z: space.z,
    ...
    const fill = bestTilingFill(front, state, context, candidate);
    const ratio = residualLength / Math.max(EPS, context.container.length);
    if (!spaceCanFitRemaining(front, state, context, candidate)) penalty += 90 * ratio;
    else penalty += (1 - fill) * 45 * ratio;
  }

  return penalty;
}

function activeFloorFrontierX(state: State, context: Context) {
  let frontier = Number.POSITIVE_INFINITY;
  for (const space of state.spaces) {
    if (space.z > TOUCH) continue;
    if (!spaceCanFitRemaining(space, state, context)) continue;
    frontier = Math.min(frontier, space.x);
  }
...
function candidateScore(candidate: Candidate, state: State, context: Context) {
  ...
  const contacts = contactMetrics(candidate, state.placements);
  const faceScale = Math.max(EPS, block.length * block.height + block.width * block.height + block.length * block.width);
  const contactRatio = clamp01(contacts.allArea / faceScale);
  const sameSkuContactRatio = clamp01(contacts.sameSkuArea / faceScale);
  const wallWidthFill = clamp01(block.width / Math.max(EPS, candidate.space.width));
  const wallHeightFill = clamp01(block.height / Math.max(EPS, candidate.space.height));
  const wallCrossFill = wallWidthFill * wallHeightFill;
  ...
  if (candidate.x <= EPS) score += 8;
  if (candidate.y <= EPS || candidate.y + block.width >= context.container.width - EPS) score += 5;

  // 같은 깊이에서 채울 수 있는 바닥 공간이 남아 있는데 앞으로 건너뛰는 배치를 강하게 억제한다.
  if (frontier !== null && candidate.z <= TOUCH && candidate.x > frontier + TOUCH) {
    const jump = (candidate.x - frontier) / Math.max(EPS, context.container.length);
    score -= 210 + jump * 260;
  }

  score -= residualPenalty(candidate, state, context);
  score -= xNorm * 2.5;
  score -= zNorm * 16;

  // 무거운 화물은 안쪽이 아니라 낮은 위치에 보상한다.
  score += weightRank * (1 - zNorm) * 32;
  score -= weightRank * zNorm * 44;
  score -= Math.abs(cogX - 0.5) * 22 + Math.abs(cogY - 0.5) * 30;

  if (context.strategy === 'capacity') score += fill * 35 + blockRatio * 240 + wallCrossFill * 40;
  if (context.strategy === 'stability') score += weightRank * (1 - zNorm) * 18 - zNorm * 38 - Math.abs(cogY - 0.5) * 24 + contactRatio * 45;
...
function xOptionsFor(space: Space, block: Block, placements: Placement[]) {
  const values = new Set<number>([round6(space.x)]);
  for (const p of placements) {
    values.add(round6(p.x + p.length));
    values.add(round6(p.x - block.length));
  }
  return [...values]
    .filter((x) => x + EPS >= space.x && x + block.length <= space.x + space.length + EPS)
    .sort((a, b) => a - b)
    .slice(0, 6);
}

function yOptionsFor(space: Space, block: Block, placements: Placement[]) {
  const values = new Set<number>([
    round6(space.y),
    round6(space.y + space.width - block.width),
  ]);
  for (const p of placements) {
    values.add(round6(p.y + p.width));
    values.add(round6(p.y - block.width));
  }
  return [...values]
    .filter((y) => y + EPS >= space.y && y + block.width <= space.y + space.width + EPS)
    .sort((a, b) => a - b)
    .slice(0, 8);
}
...
function candidateList(state: State, context: Context, allowSingles: boolean) {
  ...
  const spaces = [...state.spaces]
    .sort((a, b) => {
      const aFrontier = frontier !== null && a.z <= TOUCH && Math.abs(a.x - frontier) <= TOUCH ? 0 : 1;
...
function intersects(space: Space, p: Placement) {
  return !(
    space.x + space.length <= p.x + EPS || p.x + p.length <= space.x + EPS
    || space.y + space.width <= p.y + EPS || p.y + p.width <= space.y + EPS
    || space.z + space.height <= p.z + EPS || p.z + p.height <= space.z + EPS
  );
}
...
function subtract(space: Space, p: Placement): Space[] {
  ...
  return [
    ...
    .map((s) => ({
      ...
      y: round6(s.y),
      z: round6(s.z),
      length: round6(s.length),
      width: round6(s.width),
      height: round6(s.height),
    }));
}
...
function apply(state: State, candidate: Candidate): State {
  ...
  left.set(candidate.block.item.id, Math.max(0, (left.get(candidate.block.item.id) ?? 0) - candidate.block.quantity));
  return {
...
function voidShapePenalty(state: State, context: Context) {
  const containerVolume = Math.max(EPS, context.container.length * context.container.width * context.container.height);
  let penalty = 0;
  const spaces = [...state.spaces]
    .sort((a, b) => spaceVolume(b) - spaceVolume(a))
    .slice(0, 20);

  for (const space of spaces) {
    if (space.z > context.container.height * 0.55) continue;
    const fill = bestTilingFill(space, state, context);
    const volumeRatio = Math.min(1, spaceVolume(space) / containerVolume);
    const aspect = Math.max(space.length, space.width) / Math.max(EPS, Math.min(space.length, space.width));

    if (!spaceCanFitRemaining(space, state, context)) penalty += Math.min(38, 210 * volumeRatio);
    else penalty += (1 - fill) * 55 * volumeRatio;

    if (space.z <= TOUCH && aspect > 6 && fill < 0.75) {
      penalty += Math.min(35, (aspect - 6) * 2.5 * (1 - fill));
    }
  }
...
function stateScore(state: State, context: Context) {
  ...
  const shapePenalty = voidShapePenalty(state, context);

  let score = utilization * 850 + clamp01(demandCompletion) * 360 + countCompletion * 120
    + balance * 75 + lowCog * 70 - halfPenalty - shapePenalty - Math.min(45, state.spaces.length * 0.18);
...
function signature(state: State) {
  const remaining = [...state.remaining.entries()]
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([id, q]) => `${id}:${q}`)
    .join(',');
  const tail = state.placements
    .slice(-10)
    .map((p) => `${p.cargoId}@${p.x},${p.y},${p.z}`)
    .join(';');
...
function trim(states: State[], context: Context) {
  ...
  return [...unique.values()]
    .sort((a, b) =>
      stateScore(b, context) - stateScore(a, context)
...
export function packByBlockSpaceBeamV2(container: ContainerSpec, cargo: CargoItem[], strategy: BeamPackingStrategy): BeamPackingOutput {
  ...
  // 1차: 동일 SKU 블록으로 벽을 완성한다. 2차: 남는 안전 공간에 단품까지 허용한다.
