...
export function estimateAdditionalCargo(
  ...
  for (let i = 0; i < Math.max(1, Math.floor(maxProbe)); i += 1) {
    ...
    usedVolumeM3 += itemVolume;
  }

  if (quantity >= maxProbe) stopReason = `탐색 상한 ${maxProbe}개까지 추가 적재 가능합니다.`;
  ...
  return {
    ...
    additionalVolumeM3: quantity * itemVolume,
...
export function recommendSpareCapacity(
  ...
  return cargo
    .filter(item => item.length > 0 && item.width > 0 && item.height > 0 && item.weightKg >= 0)
