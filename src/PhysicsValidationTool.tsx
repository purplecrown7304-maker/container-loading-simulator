...
type LoadingWindow = Window & { __containerLoadingLatestResult?: LoadingDetail; __containerLoadingLatestPhysics?: PhysicsValidationSuite };
type Status = 'idle' | 'running' | 'done' | 'error';

function scoreLabel(score:number){if(score>=95)return'매우 안정';if(score>=85)return'안정';if(score>=70)return'주의';return'재배치 필요'}
function scoreClass(score:number){if(score>=95)return'excellent';if(score>=85)return'good';if(score>=70)return'warn';return'danger'}
function mm(value:number){return`${(value*1000).toFixed(value*1000>=10?0:1)} mm`}
function scenarioLabel(value:PhysicsScenario){return value==='settle'?'정적 중력':value==='braking'?'급제동 0.5g':'횡가속 0.35g'}
...
export default function PhysicsValidationTool(){
  ...
  useEffect(()=>{
    ...
    const onResult=(event:Event)=>{detailRef.current=(event as CustomEvent<LoadingDetail>).detail;if(!readPhysicsTarget()){setTargetMode('boxes');invalidate('박스 적재 결과가 변경되었습니다. 물리 검증을 다시 실행하세요.')}};
    const onTarget=(event:Event)=>{targetRef.current=(event as CustomEvent<PhysicsTarget|undefined>).detail;const mode=targetRef.current?.mode??'boxes';setTargetMode(mode);invalidate(mode==='pallets'?'팔레트 적재 결과가 변경되었습니다. 물리 검증을 다시 실행하세요.':'적재 모드가 변경되었습니다. 물리 검증을 다시 실행하세요.')};
    window.addEventListener(LOADING_RESULT_EVENT,onResult);window.addEventListener(PHYSICS_TARGET_EVENT,onTarget);return()=>{window.removeEventListener(LOADING_RESULT_EVENT,onResult);window.removeEventListener(PHYSICS_TARGET_EVENT,onTarget)};
  },[invalidate]);

  const currentTarget=useCallback(():PhysicsTarget|undefined=>{const pallet=readPhysicsTarget();if(pallet)return pallet;const detail=detailRef.current??(window as LoadingWindow).__containerLoadingLatestResult;return detail?{mode:'boxes',container:detail.container,cargo:detail.cargo,result:detail.result}:undefined},[]);
  ...
  <div className="physics-metrics"><article><span>박스 안정</span><b>{validation.stableCount} EA</b></article><article className={validation.warningCount?'warn':''}><span>박스 주의</span><b>{validation.warningCount} EA</b></article><article className={validation.unstableCount?'danger':''}><span>박스 불안정</span><b>{validation.unstableCount} EA</b></article>{validation.supports.length>0&&<><article><span>팔레트 안정</span><b>{validation.supportStableCount} EA</b></article><article className={validation.supportWarningCount?'warn':''}><span>팔레트 주의</span><b>{validation.supportWarningCount} EA</b></article><article className={validation.supportUnstableCount?'danger':''}><span>팔레트 불안정</span><b>{validation.supportUnstableCount} EA</b></article></>}<article><span>최대 수평 이동</span><b>{mm(validation.maxHorizontalShiftM)}</b></article><article><span>최대 높이 변화</span><b>{mm(validation.maxVerticalShiftM)}</b></article><article><span>최대 기울기</span><b>{validation.maxTiltDeg.toFixed(1)}°</b></article></div>
  ...
  <section className="physics-issues"><div className="physics-section-head"><div><h3>박스 재확인 위치</h3><span>{issues.length?'세 시나리오 중 가장 나쁜 결과를 기준으로 최대 30개 표시합니다.':'물리 기준을 넘은 박스가 없습니다.'}</span></div><button type="button" onClick={()=>void run()}>다시 검증</button></div>{issues.length>0&&<div className="physics-issue-list">{issues.map(item=><button key={item.index} type="button" className={item.severity} onClick={()=>{if(targetMode==='boxes')selectPlacement(item.index);setOpen(false)}}><span className="physics-issue-badge">{item.severity==='unstable'?'불안정':'주의'}</span><b>#{item.index+1} · {item.cargoId}</b><small>{item.reason}</small><i>{targetMode==='boxes'?'3D에서 보기 →':'팔레트 뷰에서 위치 확인'}</i></button>)}</div>}</section>
  {validation.supports.length>0&&<section className="physics-issues physics-support-issues"><div className="physics-section-head"><div><h3>팔레트 재확인</h3><span>{supportIssues.length?'팔레트 자체의 이동·기울기 위험을 표시합니다.':'팔레트 지지체는 모두 안정 범위입니다.'}</span></div></div>{supportIssues.length>0&&<div className="physics-support-list">{supportIssues.map(item=><article key={item.id} className={item.severity}><span>{item.severity==='unstable'?'불안정':'주의'}</span><b>{item.id}</b><small>{item.reason}</small></article>)}</div>}</section>}</>}
  {status==='error'&&<div className="physics-empty"><b>검증을 시작할 수 없습니다.</b><span>{message}</span><button type="button" onClick={()=>void run()}>다시 시도</button></div>}{status==='idle'&&<div className="physics-empty"><b>물리 검증 준비 완료</b><span>{message||'현재 적재 결과에 중력·급제동·횡가속을 적용합니다.'}</span><button type="button" onClick={()=>void run()}>종합 물리 검증 시작</button></div>}<footer className="physics-footnote">0.5g 급제동과 0.35g 횡가속은 비교·경고용 기본 시나리오입니다. 실제 운송 안전 판정에는 차량·도로·고정장치·마찰계수·포장재 강도 등 회사/법규 기준을 별도로 적용해야 합니다.</footer></section></div>,document.body)}
