  ...
  const execute = useCallback(async (_detail: ResultsModalDetail) => {
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      if (certification.status === 'passed') {
        ...
        setRunning(false);
        setMessage(`최종 PASS · ${candidate.label}${axle?.severity === 'warning' ? ' · 축하중 주의 범위' : ''}`);
        setOpen(false);
        openResultsModal({
    ...
    setError(axleRejected > 0
      ? `팔레트 위치·상자 패턴·높이 조합을 모두 시험했지만 관성 3종과 입력된 트럭 축하중 조건을 동시에 만족하지 못했습니다. 축하중 초과로 제외된 후보 ${axleRejected}개가 있습니다.`
      : '화물 수량과 제약조건을 유지하면서 만들 수 있는 팔레트 위치·팔레트 위 상자 패턴·높이 조합을 모두 시험했지만 관성 3종 PASS가 나오지 않았습니다. 결과 보기는 잠금 상태를 유지합니다.');
  }, []);
    ...
    <section className="final-cert-modal" role="dialog" aria-modal="true" aria-labelledby="pallet-result-opt-title">
      <header>
        <div>
          ...
          <p>PASS가 나올 때까지 팔레트 위치뿐 아니라 팔레트 위 상자의 패턴·높이·적층단까지 바꾸어 관성 3종과 설정된 트럭 축하중을 다시 시험합니다.</p>
        </div>
      ...
      {progress && <div className="final-cert-metrics">
        ...
        <span>종료 조건 <b>관성 + 축하중 PASS 또는 유효 배치 소진</b></span>
      </div>}
      ...
      <article className="final-cert-materials">
        ...
        <div className="final-cert-material-grid">
          <div><span>팔레트 위 패턴</span><b>블록/벽돌/스플릿/핀휠</b><small>지원면을 검사한 실제 좌표 후보</small></div>
          ...
          <div><span>적재함 폭</span><b>2열 우선</b><small>가능하면 좌우 동시 배치</small></div>
          <div><span>트럭 축하중</span><b>팔레트 자중 포함</b><small>초과 후보는 자동 제외</small></div>
          <div><span>승인</span><b>관성 3종 + 축하중</b><small>조건 충족 전 결과 잠금</small></div>
        </div>
      </article>
