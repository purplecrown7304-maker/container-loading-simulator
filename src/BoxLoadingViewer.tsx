...
function CargoInstances({ items, container, scale }: { items: IndexedPlacement[]; container: ContainerSpec; scale: number }) {
...
function Scene({ result, container }: { result: LoadingResult; container: ContainerSpec }) {
  ...
  return <>
    ...
    {grouped.map(([cargoId, items]) => <CargoInstances key={cargoId} items={items} container={container} scale={scale} />)}
...
export default function BoxLoadingViewer({ result, container }: { result: LoadingResult; container: ContainerSpec }) {
  return <section className="viewer">
    <Canvas camera={{ position:[5.5,4.2,6.5], fov:48 }} dpr={[1, 1.5]} gl={{ antialias: true, powerPreference: 'high-performance' }}>
      <Scene result={result} container={container} />
    </Canvas>
    <div className="viewer-direction"><b>박스 적재</b><span>안쪽 → 문쪽 · Instanced 3D</span></div>
  </section>;
}
