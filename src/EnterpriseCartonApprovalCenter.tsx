...
import { approveGeneratedCarton, verifiedStackLayers } from './engine/cartonStrengthApproval';
...
export default function EnterpriseCartonApprovalCenter() {
  ...
  const [draft, setDraft] = useState<VerificationDraft>({ catalogId: '', maxTopLoadKg: '', maxGrossWeightKg: '', unitCost: '', stepMm: 5 });
  ...
  const refresh = () => {
    ...
    setDraft({
      catalogId: first ? `VER-${first.boxId}` : '',
      maxTopLoadKg: '',
      maxGrossWeightKg: first ? Math.ceil(first.grossWeightKg * 10) / 10 + '' : '',
      unitCost: '',
      stepMm: 5,
    });
  ...
  const choose = (productId: string) => {
    ...
    setDraft({
      catalogId: assignment ? `VER-${assignment.boxId}` : '',
      maxTopLoadKg: '',
      maxGrossWeightKg: assignment ? Math.ceil(assignment.grossWeightKg * 10) / 10 + '' : '',
      unitCost: '',
      stepMm: draft.stepMm,
    });
  };
  ...
  const approve = () => {
    ...
    const stack = verifiedStackLayers(selected, maxTopLoadKg);
    setMessage(`${result.box.id} 검증 박스를 회사 카탈로그에 등록했습니다. 검증값 기준 최대 ${stack}단 후보입니다. 전체 포장 최적화를 다시 실행하세요.`);
    setStored(next);
    setAssignments((items) => items.filter((item) => item.productId !== selected.productId));
    setSelectedId('');
    setDraft((value) => ({ ...value, catalogId: '', maxTopLoadKg: '', maxGrossWeightKg: '', unitCost: '' }));
  };

  return <section className="enterprise-approval-center" aria-label="자동설계 박스 제조 강도 승인">
    <div className="approval-head">
      <div><span>PACKAGING VERIFICATION</span><h3>자동설계 박스 제조 검증 승인</h3><p>자동 계산된 요구강도가 아니라 제조사/포장시험에서 확인한 실제 값을 입력해야 보유박스로 승격됩니다.</p></div>
      <button type="button" onClick={refresh}>승인 대기 규격 불러오기</button>
    </div>

    {assignments.length > 0 && <div className="approval-grid">
      <div className="approval-pending-list">{assignments.map((item) => <button type="button" key={item.productId} className={selectedId === item.productId ? 'active' : ''} onClick={() => choose(item.productId)}><b>{item.productId}</b><span>{item.boxName}</span><small>{mm(item.outerLength)}×{mm(item.outerWidth)}×{mm(item.outerHeight)}mm · 요구 {item.requiredTopLoadKg.toFixed(0)}kg</small></button>)}</div>
      {selected && product && <div className="approval-form">
        <div className="approval-design-summary"><b>{selected.productName}</b><span>현재 Full {selected.grossWeightKg.toFixed(2)}kg · 설계목표 {selected.recommendedStackLayers}단</span><span>계산상 요구 상부하중 {selected.requiredTopLoadKg.toFixed(1)}kg</span></div>
        <label>승인 박스 코드<input value={draft.catalogId} onChange={(event) => setDraft((value) => ({ ...value, catalogId: event.target.value }))} /></label>
        <label>제조사 검증 최대총중량(kg)<input type="number" min="0.01" step="0.1" value={draft.maxGrossWeightKg} onChange={(event) => setDraft((value) => ({ ...value, maxGrossWeightKg: event.target.value }))} /></label>
        ...
        <small>승인 후 외경은 선택한 제조 단위로 올림됩니다. 내부 제품 수용공간은 줄이지 않습니다.</small>
      </div>}
    </div>}
