...
export type UnityDecoration = { x: number; y: number; z: number; length: number; width: number; height: number; color: string; supportIndex: number; wire?: boolean };
export function securingGeometry(container: ContainerSpec, boxes: Placement[], supports: PhysicsSupport[], usage?: SecuringUsage | null): UnityDecoration[] {
  ...
  const add = (x: number, y: number, z: number, length: number, width: number, height: number, color: string, supportIndex = -1, wire = false) => output.push({ x, y, z, length, width, height, color, supportIndex, wire });
  ...
  supports.forEach((p, index) => {
    ...
    if (usage.cornerGuards) for (const x of [p.x, p.x + p.length - .035]) for (const y of [p.y, p.y + p.width - .035]) add(x, y, base, .035, .035, height, '#d6b276', index);
  ...
  if (!supports.length) {
    ...
    for (let i = 0; i < usage.dunnageBlocks; i++) add(minX + (maxX - minX) * (Math.floor(i / 2) + 1) / (Math.ceil(usage.dunnageBlocks / 2) + 1), i % 2 ? Math.min(container.width - .09, maxY) : Math.max(0, minY - .09), 0, .22, .09, .22, '#b78650');
  }
