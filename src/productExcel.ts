...
import type { ProductItem, ProductOrientationPolicy } from './engine/productPackagingOptimizer';

export type ProductImportIssue = { row: number; code?: string; message: string };
export type ProductImportResult = { items: ProductItem[]; issues: ProductImportIssue[]; totalRows: number };
...
export function downloadProductTemplate() {
  const worksheet = XLSX.utils.aoa_to_sheet([
    HEADERS,
    ['PRD-001', '제품 A', 220, 150, 100, 1.2, 100, 24],
    ['PRD-002', '제품 B', 310, 180, 120, 2.5, 60, 12],
  ]);
  worksheet['!cols'] = [16, 24, 13, 13, 13, 13, 13, 17].map((wch) => ({ wch }));
