...
import { workOrderCertificationForTarget } from './workOrderCertificationSnapshot';
...
export function buildPalletLoadingReportHtml(container: ContainerSpec, cargo: CargoItem[], snapshot: PalletWorkSnapshot, certification: InertiaCertification): string {
  ...
  const badgeClass = approval === 'caution' ? 'pass caution' : approval === 'danger' ? 'pass danger' : approval === 'incomplete' ? 'pass incomplete' : 'pass';
  const recommendationTone = approval === 'danger' ? '#fecaca' : approval === 'caution' || approval === 'incomplete' ? '#fed7aa' : '#bfdbfe';
  const recommendationBg = approval === 'danger' ? '#fff1f2' : approval === 'caution' || approval === 'incomplete' ? '#fff7ed' : '#eff6ff';
  const palletTareWeightKg = snapshot.result.palletCount * snapshot.spec.tareWeightKg;
  const packagingWeightKg = snapshot.result.totalPackagingWeightKg;
  const securingWeightKg = certification.securing.estimatedAddedWeightKg;
  const finalTransportWeightKg = snapshot.result.totalPalletizedWeightKg + securingWeightKg;
  const safeToExecute = approval === 'pass' || approval === 'caution';

  return `<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>팔레트 적재 작업지시서</title><style>
  @page{size:A4 portrait;margin:8mm}*{box-sizing:border-box}body{font-family:Arial,"Noto Sans KR",sans-serif;color:#172033;margin:0;font-size:10px}h1,h2,p{margin:0}.sheet{max-width:794px;margin:0 auto}.header{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;border-bottom:3px solid #172033;padding-bottom:8px}.header h1{font-size:23px}.header p{margin-top:4px;color:#64748b}.pass{padding:7px 11px;border:2px solid #16a34a;border-radius:10px;background:#f0fdf4;color:#166534;text-align:center}.pass.caution,.pass.incomplete{border-color:#d97706;background:#fff7ed;color:#9a5b00}.pass.danger{border-color:#dc2626;background:#fff1f2;color:#b91c1c}.pass b{display:block;font-size:15px}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin:8px 0}.summary div{padding:7px;border:1px solid #cbd5e1;border-radius:8px;background:#f8fafc}.summary span{display:block;color:#64748b;font-size:8px}.summary b{display:block;margin-top:2px;font-size:12px}.direction{padding:7px;text-align:center;border-radius:8px;background:#eff6ff;color:#1d4ed8;font-size:12px;font-weight:900;margin-bottom:6px}.diagrams{display:grid;grid-template-columns:1fr;gap:6px}.diagrams svg{width:100%;height:auto;display:block;border:1px solid #e2e8f0;border-radius:9px}.title{display:flex;justify-content:space-between;align-items:end;margin:9px 0 4px}.title h2{font-size:14px}.title span{font-size:8px;color:#64748b}.work{width:100%;border-collapse:collapse}.work th,.work td{border:1px solid #cbd5e1;padding:5px;vertical-align:middle}.work th{background:#172033;color:#fff;font-size:8px}.work small{display:block;margin-top:2px;color:#64748b;font-size:7.5px}.seq{width:34px;text-align:center;background:#eff6ff;color:#1d4ed8}.seq b{font-size:15px}.check{width:32px;text-align:center;font-size:18px}.sequence{display:grid;grid-template-columns:repeat(2,1fr);gap:6px}.sequence article{padding:7px;border:1px solid #cbd5e1;border-radius:8px;background:#f8fafc;break-inside:avoid}.sequence article.shared{border-color:#fed7aa;background:#fff7ed}.sequence header{display:flex;justify-content:space-between;gap:6px;border-bottom:1px solid #e2e8f0;padding-bottom:4px}.sequence header b{font-size:11px;color:#1d4ed8}.sequence header span{font-size:7.5px;color:#64748b}.sequence p{padding:3px 0;border-bottom:1px dotted #dbe3ee;font-size:8px}.sequence footer{margin-top:4px;text-align:right;font-size:7.5px;font-weight:800;color:#52617a}.materials{display:grid;grid-template-columns:repeat(3,1fr);gap:5px}.materials div{padding:6px;border:1px solid #cbd5e1;border-radius:7px}.materials span,.materials i{display:block;font-size:8px;color:#64748b}.materials b{display:block;margin:2px 0;font-size:11px}.materials i{font-style:normal;color:#166534}.history{display:flex;gap:5px;flex-wrap:wrap}.history span{display:flex;flex-direction:column;gap:2px;padding:5px 7px;border-radius:7px;font-size:7.5px;border:1px solid #e2e8f0;background:#f8fafc}.history .pass-step{border-color:#bbf7d0;background:#f0fdf4}.history .fail-step{border-color:#fed7aa;background:#fff7ed}.history b{font-size:8px}.inertia-metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:5px}.inertia-metrics div{padding:6px;border:1px solid #bfdbfe;border-radius:7px;background:#eff6ff}.inertia-metrics span{display:block;font-size:7.5px;color:#64748b}.inertia-metrics b{display:block;margin-top:2px;font-size:10px;color:#172033}.recommendations{display:grid;grid-template-columns:1fr 1fr;gap:5px;margin:0;padding:0}.recommendations li{list-style:none;display:grid;grid-template-columns:23px 1fr;gap:6px;padding:6px;border:1px solid ${recommendationTone};border-radius:7px;background:${recommendationBg};font-size:8px;line-height:1.45}.recommendations li b{display:grid;place-items:center;width:21px;height:21px;border-radius:50%;background:#fff;border:1px solid #cbd5e1;color:#1d4ed8}.final{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-top:7px}.final div{padding:7px;border:1px solid #f0b85f;border-radius:7px;background:#fff8e8;font-weight:800}.note{margin-top:7px;padding:7px;border-left:4px solid #2563eb;background:#eff6ff;font-size:9px;line-height:1.5}.technical{margin-top:5px;font-size:7.5px;color:#64748b;line-height:1.4}.footer{display:flex;justify-content:space-between;gap:7px;margin-top:7px;padding-top:5px;border-top:1px solid #cbd5e1;font-size:7.5px;color:#64748b}.watermark{position:fixed;inset:42% 0 auto;text-align:center;transform:rotate(-18deg);font-size:46px;font-weight:900;color:rgba(185,28,28,.09);pointer-events:none;z-index:20}@media print{.watermark{display:${physicsVerified && safeToExecute ? 'none' : 'block'}}}
  </style></head><body>${physicsVerified && safeToExecute ? '' : '<div class="watermark">검증/경고 확인 필요</div>'}<main class="sheet"><header class="header"><div><h1>팔레트 적재 작업지시서</h1><p>${escapeHtml(generatedAt)} · 관성 상태와 관계없이 현재 팔레트 적재안을 기준으로 발행</p></div><div class="${badgeClass}"><span>관성 3종</span><b>${escapeHtml(approvalLabel)}</b></div></header><section class="summary"><div><span>팔레트 / 화물</span><b>${snapshot.result.palletCount} / ${snapshot.result.placements.length} EA</b><small>미적재 ${escapeHtml(remaining)}</small></div><div><span>화물 중량</span><b>${snapshot.result.loadedCargoWeightKg.toFixed(1)} kg</b></div><div><span>팔레트 자중</span><b>${palletTareWeightKg.toFixed(1)} kg</b></div><div><span>기본 포장재</span><b>${packagingWeightKg.toFixed(1)} kg</b></div><div><span>팔레트화 중량</span><b>${snapshot.result.totalPalletizedWeightKg.toFixed(1)} kg</b></div><div><span>운송 보강재</span><b>${securingWeightKg.toFixed(1)} kg</b></div><div><span>최종 운송중량</span><b>${finalTransportWeightKg.toFixed(1)} kg</b></div><div><span>최대 적층</span><b>${snapshot.result.maxUsedStackLevel} / ${snapshot.spec.maxStackLevels}단</b></div></section><div class="direction">◀ 안쪽부터 · 1단 먼저 · 같은 C번호의 상단 팔레트 적층 · 결속카드 순서대로 작업 · 문쪽 ▶</div><section class="diagrams">${top}${side}</section><div class="title"><h2>팔레트 투입 순서</h2><span>한 줄 완료할 때마다 □ 체크</span></div><table class="work"><thead><tr><th>순서</th><th>팔레트</th><th>박스 구성</th><th>중량/고정</th><th>놓을 위치</th><th>완료</th></tr></thead><tbody>${rows}</tbody></table><div class="title"><h2>팔레트별 결속 작업 순서</h2><span>미끄럼방지 → 배치 → 각대 → 밴딩 → 랩핑 → 확인</span></div><section class="sequence">${sequence}</section>${history ? `<div class="title"><h2>자동 보강 이력</h2><span>기본안부터 최종 판정까지</span></div><section class="history">${history}</section>` : ''}<div class="title"><h2>관성 안전 지표</h2><span>팔레트 모드는 상대 미끄럼을 별도 확인</span></div><section class="inertia-metrics">${inertia}</section><div class="title"><h2>관성 테스트 권장 사항</h2><span>${approval === 'danger' ? '위험 경고 · 수정 후 재검증 필수' : approval === 'incomplete' ? '검사 미완료 · 남은 검사 필요' : approval === 'caution' ? '주의 · 출고 전 보완 확인 필수' : '현장 최종 확인 사항'}</span></div><ol class="recommendations">${recommendationItems}</ol><div class="title"><h2>필요 보조자재 총량</h2><span>${escapeHtml(certification.securing.levelLabel)}</span></div><section class="materials">${materials}</section><div class="final"><div>□ 밴딩/각대/랩핑 그림과 일치</div><div>□ 팔레트 흔들림·오버행 없음</div><div>□ 문 닫힘/고정바 간섭 없음</div></div><p class="note"><b>작업자가 기억할 것:</b> 팔레트는 P번호 순서로 넣고, 같은 C번호는 같은 수직 적층 위치입니다. <b>1단을 먼저 놓은 뒤 같은 C번호의 상단 팔레트를 순서대로 올리세요.</b> 검은 선=밴딩, 갈색 모서리=각대, 파란 점선=랩핑, 주황선=고정바입니다.</p><p class="technical">이 문서는 관성 PASS 여부와 관계없이 현재 적재 위치를 전달하기 위해 발행됩니다. 관성 ${escapeHtml(approvalLabel)}은 시뮬레이터 내부 비교 결과이며 실제 운송 안전 인증을 의미하지 않습니다. 위험 또는 검사 미완료 표시는 출고 승인과 다르므로 화물↔팔레트 미끄럼, 적층 팔레트 상대 이동, 실제 자재 정격과 현장 기준을 출고 전 확인하세요.</p><footer class="footer"><span>물리검증: ${physicsVerified ? '완료' : '별도 확인'}</span><span>관성: ${escapeHtml(approvalLabel)} · 전체 ${(certification.maxHorizontalShiftM * 1000).toFixed(1)}mm · 화물 ${((certification.maxCargoRelativeSlipM ?? 0) * 1000).toFixed(1)}mm · 팔레트 ${((certification.maxSupportShiftM ?? 0) * 1000).toFixed(1)}mm · ${certification.maxTiltDeg.toFixed(1)}°</span><span>최종 운송중량 ${finalTransportWeightKg.toFixed(1)}kg</span></footer></main></body></html>`;
}
...
export function openPalletLoadingReport(container: ContainerSpec, cargo: CargoItem[]): boolean {
  ...
  const target = physicsTargetFromPalletSnapshot(container, cargo, snapshot);
  const certification = workOrderCertificationForTarget(target);
