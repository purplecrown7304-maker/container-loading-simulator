  ...
  return attempts.map((attempt, index) => {
    ...
    const result = attempt.passed
      ...
      : !attempt.payloadWithinLimit
        ? '보조재 포함 최대중량 초과'
        : failed
          ? `${failed.scenario === 'acceleration' ? '출발 가속' : failed.scenario === 'braking' ? '급정거' : '급회전'} FAIL · 전체 ${(failed.maxHorizontalShiftM * 1000).toFixed(1)}mm · 화물↔팔레트 ${((failed.maxCargoRelativeSlipM ?? 0) * 1000).toFixed(1)}mm · 팔레트 ${((failed.maxSupportShiftM ?? 0) * 1000).toFixed(1)}mm · 기울기 ${failed.maxTiltDeg.toFixed(1)}°`
          : '검증 미완료';
...
export function openPalletLoadingReport(container: ContainerSpec, cargo: CargoItem[]): boolean {
  ...
  if (!certification) {
    window.alert('팔레트 작업지시서는 현재 팔레트 적재안이 출발 가속·급정거·급회전 관성 시뮬레이션 3종을 모두 통과하고 인증 좌표·팔레트 규격과 출력 Snapshot이 일치한 뒤 출력할 수 있습니다.');
    return true;
  }
