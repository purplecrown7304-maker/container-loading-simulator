...
[Serializable] public class CargoPlan { public int revision; public SpaceSpec container; public string geometry; public CargoBox[] placements,supports; public Decoration[] decorations; public WeightCell[] cells; public CargoPoint centerOfGravity; }
...
public class CargoViewer : MonoBehaviour {
 ...
 public void SetPlan(string json) {
  ...
  if(plan.geometry!="platform") Cube("Back wall",new Vector3(-l/2-.025f,h/2,0),new Vector3(.05f,h,w),wall,shell);
  if(plan.geometry!="platform"&&plan.geometry!="flat-rack") Cube("Far wall",new Vector3(0,h/2,-w/2-.025f),new Vector3(l,h,.05f),wall,shell);
  if(plan.geometry!="platform"&&plan.geometry!="flat-rack") for(float x=-l/2;x<l/2;x+=.3f) Cube("Corrugation",new Vector3(x,h/2,-w/2+.01f),new Vector3(.025f,h,.025f),floor,shell);
  ...
  for(int i=0;i<plan.placements.Length;i++) {
   ...
   var go=Cube("Cargo_"+i,new Vector3(p.x+p.length/2-l/2,p.z+p.height/2,p.y+p.width/2-w/2),new Vector3(p.length,p.height,p.width)*.99f,palette[key],cargoRoot,true); boxes.Add(go);
  }
  supportRoot=new GameObject("Pallets").transform; decorRoot=new GameObject("Securing").transform;
  var timber=Mat(new Color(.77f,.56f,.32f));
  foreach(var p in plan.supports??new CargoBox[0]) {
   ...
   Cube("Support_"+index,Vector3.zero,new Vector3(p.length,p.height,p.width),timber,unit.transform,true); supports.Add(unit);
  }
  foreach(var p in plan.decorations??new Decoration[0]) {
   ...
   var size=new Vector3(p.length,p.height,p.width); var mat=Mat(c);
   if(p.wire)EdgeBox(Vector3.zero,size,mat,unit.transform,.008f);else Cube("Securing",Vector3.zero,size,mat,unit.transform);
